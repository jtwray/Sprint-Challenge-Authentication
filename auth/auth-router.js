...
router.post('/login',(req, res) => {
  ...
  Users.findBy({ username }).first().then(user => {
    if (user & bcrypt.compareSync(password, user.password)) {
      const token = signToken(user); res.status(200).json({userId:`${saved.id}` , username:`${saved.username}` , token:`${token} `});
    } else {
      res.status(401).json({ message: `${Object.entries(user)} || ---Invalid Credentials--- ||` });
    }
